...
.DESCRIPTION
    Gets the existing IP address list, in Supported Software, for a customer in a given environment.
.OUTPUTS
    Messages to the host
.EXAMPLE
    PS> .\Get-RestIPFilterList.ps1 -EnvironmentName Test -IdCode xzy -Credential (Get-Credential) 
    Get the IP address list for customer xzy. 
.EXAMPLE
    PS> .\Get-RestIPFilterList.ps1
    You'll be asked to provide all required information, and get the IP address list for a customer
.NOTES
    Get-RestIPFilterList.ps1 is a frontend for the RestIPFilter module. This allows you to make basic changes 
    for one Supported Software customer without having to interact with the module or knowing out to use each of the imported
    cmdlets.
...
param (
    ...
    , # Specifies the customer ID code. If not entered you will be prompted for it
    [Parameter(Position=1)][string]$IdCode 
    , # Credential (username and password) for Supported Software. You may create this using Get-Credential. Might be 
...
try {
    ...
    if (-not $IdCode) {
        $IdCode = (
            New-VIFObject -EnvironmentName $EnvironmentName -UseReferenceSitecode | 
    ...
    New-VIFObject -EnvironmentName $EnvironmentName -IdCode $IdCode | 
        New-VIFAuthToken -Credential $Credential | Find-VIFCustomerInfo | Get-VIFIpAddressList -ShowIPList > $null
} catch {
